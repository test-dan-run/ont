...
def gen_colored_gaussian_noise(file_path='./sample_audio.wav', snr=10, color='white', rate=16000):

    # Load audio data into a 1D numpy array
    # fs, un_noised_file = wavfile.read(file_path)
    un_noised_file, fs = librosa.load(file_path, sr=rate)
    #un_noised_file = un_noised_file.numpy()
    # un_noised_file = np.reshape(un_noised_file, -1)
    '''
    # Create an audio Power array
...
def load_audio_file(file_path='./sample_audio.wav', rate=16000):
    #waveform, _ = torchaudio.load(file_path)
    #waveform = waveform.numpy()
    #waveform = np.reshape(waveform, -1)
    waveform, fs = librosa.load(file_path, sr=rate)
    #print(waveform)
    return waveform

def save_audio_file(np_array=np.array([0.5]*1000),file_path='./sample_audio.wav', sample_rate=16000, bit_precision=16):
    np_array = np_array.flatten()
    # np_array = np_array.astype('int16')
    sf.write(file_path, np_array, sample_rate)
    #print(np_array)
